...
public class questionTen2 {
  ...
  public static void main(String[] args){
    ...
    //printThreebyThreeGeneral(scores);

    int randMatrix[][][] = createMatrix(3, 2, 3);

    printThreebyThreeGeneral(randMatrix);


  }
  public static int[][][] createMatrix(int rows, int columns, int pairs){
    int[][][] newMatrix = new int[rows][columns][pairs];

    for (int i= 0; i< rows; i++){ // start at rows
      for (int j=0; j < columns; j++ ){ // now each column
        for (int k=0; k < pairs; k++){ // now looking at each value
          int randInt = (int)(Math.random() * 100);
          newMatrix[i][j][k] = randInt;
        }
      }
    }
